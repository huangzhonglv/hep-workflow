...
description: >
  ...
  Coordinates project state, checks prerequisites, dispatches skills,
...
Your job is to coordinate skills, read project state from `manifest.json`,
route state changes through documented writers, check prerequisites, detect
staleness, and keep the user informed.
...
**You DO:**
- Read `manifest.json` and validate owner-published state
- Dispatch the documented skill/script owner for state changes
...
Before using any status/history value to skip work or report completion, run
the workspace validator and verify exact-byte dependency graphs for completed
calculations plus the strict scan pair, summary, and configured figure coverage
for numerics. Status strings, history entries, existence, nonempty files, and
mtime are routing hints only; they are not completion evidence.
...
5. **all calculations complete** -> inspect
   ...
   - Treat `analyses[]` as per-analysis ownership objects keyed by their unique
     `analysis_id`; each entry owns its status, files, and dependencies.
   - If any entry has a scan config but is missing
...
3. For each task not yet completed (in order of `priority`: high -> medium -> low):
   ...
   f. Verify that the package finalizer atomically published the task-scoped
      manifest event; do not perform a second manifest read-modify-write
...
- hep-numerics -> numerical scan + figures, with sub-intent:
  - new analysis: "run hep-numerics", "run a scan", "analyze XXX",
    "run one scan", "run numerics"
  - rerun: "rerun", "rerun analysis-002", "rescan after config changes",
    "run again", "rerun after config changes"
...
| hep-numerics (rerun) | new analysis prerequisites + existing `numerics/scan-configs/{analysis_id}.json` | existing scan outputs for comparison |
...
3. If stale, block new downstream execution and explain which exact dependency
   hash/version/graph is stale. The user may inspect historical output, but
   cannot override the honesty gate to publish a new result.
4. Do NOT auto-rerun. Offer recomputation or an explicit legacy-unverified
   inspection path; wait for the user's choice.
...
- New analysis: ask the user to confirm the `analysis_id`, or accept the id
  ...
  outputs and require every configured figure pair; an empty `figures/` is
  complete only when the config declares no figures.
- Rerun: before dispatch, confirm that the `analysis_id` exists in
  `analyses[]` and warn that this will overwrite the old
  `scan-results/{analysis_id}/` outputs. After dispatch, verify the new strict
  pair, exact CSV hash, dependency graph, summary counts, and configured figure
  pairs; mtime is not evidence.
- Replot: before dispatch, verify the full scan pair and require the live
  execution projection to equal the frozen `scan_config_snapshot`. Validate
  `figures.meta.json`, its full live render snapshot/input graph/output hashes,
  and every configured PDF/PNG pair. `scan.meta.json` remains byte-identical
  with its original history action; `numerics_figures_regenerated` is recorded in `manifest.history[]`.
...
4. Read `artifacts.numerics.analyses[]`; for each entry's `analysis_id`, read
   `numerics/scan-configs/{analysis_id}.json` and, if present,
   `numerics/scan-results/{analysis_id}/scan.meta.json`
5. Only after strict validation, label each analysis from
   `scan.meta.json.history_action`:
...
6. Check each `analyses[].depends_on` against the active model and current
   constraints. Report an entry with `status = stale` as historical evidence,
   never as current completion. Aggregate numerics status is conservative: one
   stale/blocked/failed/partial entry cannot be hidden by another done entry.
...
1. Machine-readable canonical names must match
   `^[A-Za-z_][A-Za-z0-9_]*$`, must not be Python hard keywords, and reuse exact
   `model/model-spec.json` names; see
   `docs/contracts/canonical-name-convention.md`.
2. Read `model-spec.json.parameters[*].name` into a set and check every
   parameter reference in `calc-tasks.json`, `result-meta.json`,
   `constraints-data.json`, and the active scan config against it. Run
   `python3 scripts/validate_workspace_projects.py <project-name>` for
   cross-file and custom-observable signature checks; do not infer names from
   `numerics/custom_observables.py` or expect an `observables` list in
   `scan.meta.json`.
3. If any unrecognized parameter name is found:
   - Report the mismatch to the user
   - Do not accept completion or patch `manifest.json`; require the documented
...
After a skill completes, validate its outputs and owner-published manifest
state before accepting completion or continuing:
...
After `hep-idea` returns:
- Require its foundation finalizer result to be `published` or
  `already_published`; a generated private candidate is not completion
- Re-read `manifest.json` and require `manifest_version = 2`
- Independently compute the SHA-256 of `model/model-spec.json` and verify that
  it equals `artifacts.model.checksum`; do not store it yourself
- Verify `active_model_version = "v1"`, the idea/model/constraints artifact
  statuses, exact file lists, and `hep-idea` producer fields against the
  skill's `references/manifest-json-contract.md`
- Verify the exact initial calculations/numerics skeletons, including empty
  `completed_tasks`, `pending_tasks`, numerics `files`, and `analyses`
- Verify the `idea_complete`, `model_complete_v1`, and `constraints_complete`
  history entries and run `scripts/validate_workspace_projects.py`
...
The orchestrator must not compute and store the checksum, populate task arrays,
append history, copy candidate files, or write `manifest.json` after dispatch.
If verification fails, report the publication as incomplete and have the
documented owner correct its candidate/publication.
...
- Require the package finalizer result to be `published` or
  `already_published`, then re-read `manifest.json` and verify that:
  - `task_id` is in `calculations.completed_tasks` and absent from
    `pending_tasks`
  - `calculations.depends_on.model` matches the active model version/checksum
  - the finalizer's unique `event_id` appears exactly once with
    `calc_task_{task_id}_complete` for a first result or
    `calc_task_{task_id}_revised` for a replacement
    (for example, `calc_task_task-001_complete` for `task-001`)
  - the finalizer wrote `manifest.json` last in the same transaction as the
    complete task directory

Package-Scribe's mechanical finalizer owns this calculation manifest merge.
The orchestrator must not move task IDs, append the calculation history event,
or write `manifest.json` again after dispatch. `calculations_updated` remains
reserved for a separately authorized legacy/manual aggregate update spanning
multiple tasks, with a note naming the affected tasks.
...
After `hep-idea` returns:
- Require its foundation finalizer result to be `published` or
  `already_published`; do not inspect a private candidate as authoritative
- Run canonical name validation on every updated artifact
- Re-read the manifest and compare the requested artifact scope with the
  pre-dispatch state
- If model-spec.json changed, verify that `active_model_version`,
  `model.version`, `model.checksum`, and `model.produced_by` were updated by
  `hep-idea`, and that the model version incremented when the checksum changed
- If constraints changed, verify that `constraints.depends_on.model` contains
  the current model version and checksum and that `constraints.produced_by` is
  `"hep-idea"`
- Verify that every newly appended hep-idea history entry uses the action for
  its actual changed scope: `model_complete_{version}` or `model_updated`,
  `constraints_updated`, or `benchmarks_updated`
- After a `model-spec.json`, `calc-tasks.json`, or `benchmarks.json` revision,
  verify that the foundation finalizer marked evidence-bearing calculations
  `stale` while preserving their task registry, producer, timestamp, and prior
  model dependency. After any model or constraints revision, also verify that
  every affected numerics analysis is `stale`, the aggregate was rederived,
  historical files/dependencies were preserved, and no downstream work reran
- For a legacy/external upstream publication that predates this finalizer, run
  the read-only diagnostic:
  `python3 scripts/refresh_numerics_staleness.py --project-dir <project-dir>`.
  If it reports `NEEDS REFRESH`, invoke the same command with `--write`, then
  verify the exact narrow projection. Never use this repair path to compensate
  for invalid model/constraints metadata; the script must fail closed there
- If the required stale projection still has not been owner-published, stop
  and report the workflow as blocked; do not patch it as the orchestrator
- Run `scripts/validate_workspace_projects.py` before accepting the revision

The requested idea/model/constraints/benchmark content and candidate
fields/history are owned by `hep-idea`; mechanical publication and derived
staleness are script-owned. The orchestrator must not repair fields, append
history, copy candidate paths, or perform a second manifest write. If
verification fails, report the mismatch and have the documented owner correct
its publication.
...
- `numerics/scan-results/{analysis_id}/scan.meta.json` (valid against
  ...
  `history_action`, exact config source/hash/snapshot, explicit `rng`, counts,
  `formula_fallbacks`, and `warnings`)
- Recompute `scan_csv_sha256`, require the config-derived exact header and one
  complete unique finite Cartesian grid, verify counts/summary markers, and
  independently verify `input_provenance` exact coverage and hashes.
- `numerics/analysis-summary-{analysis_id}.md` (non-empty Markdown)
- Every figure declared by the scan config requires its collision-free PDF/PNG
  pair plus verified `figures.meta.json` ownership before numerics can be
  `done`, in every branch. A config with no figures may complete with an empty
  figure directory.
- If `numerics/custom_observables.py` exists, verify that the numerics writer
  included it in the manifest file projection; do not require it for every
  analysis.
...
Any other `history_action` value is an error; reject the owner publication and
do not patch it. When reading `manifest.history[]`, validate each `action`
against `schemas/manifest.schema.json`. Model-first actions this orchestrator
must recognize and validate are:
...
- `hep-idea` owns idea, model, constraints, and benchmark actions
- `finalize_package_result.py` owns task-scoped calculation actions
- `run_scan.py` / `make_figures.py` own numerics actions
- `calculations_updated` is reserved for a separately authorized legacy/manual
  aggregate writer; recognizing it does not authorize this orchestrator to emit
  it

The following schema-valid actions are external to this orchestrator:
- `literature_complete` and `literature_updated`, owned by
  `hep-paper-formalize`
- `reproduction_run_complete`, owned by `compare_to_reference.py`
- `reproduction_run_failed`, reserved for a future contract-bound mechanical
  failure recorder; `repro-orchestrator` validates but never writes either
  reproduction action

Recognize these actions when reading a shared manifest; do not emit them and do
not reject a manifest solely because it contains them. Accept owner publications
only for `manifest_version = 2`. The explicit
`scripts/migrate_manifest_v2.py --write` path owns migration after its read-only
check; the orchestrator never silently reinterprets or directly migrates legacy
global dependencies. When validating a v2 numerics publication, require the
writer to upsert only the current `analysis_id` and preserve every other
`numerics.analyses[]` entry's files, dependencies, producer, and timestamp. The
only permitted unrelated status change is the conservative `done`/`partial` to
`stale` transition when exact inputs drift.
Each analysis entry owns `status`,
`files`, `depends_on`, `produced_by`, and `timestamp`; there is no global
numerics dependency object because heterogeneous analyses may use different
model versions and task sets. Aggregate `numerics.files` is the exact sorted
union of entry files, and aggregate status is derived conservatively. The
replot writer may update only the addressed entry's figure ownership/status and
must preserve its scan dependencies as well as every unrelated entry.
Every new numerics history entry has an explicit `analysis_id` and a globally
fresh 32-character lowercase-hex `event_id`; missing fields/note-token fallback
are legacy-read compatibility only.
Hep-numerics builds the merged candidate through
`scripts/_manifest.py:build_manifest_for_numerics(...)`; `run_scan.py` /
`make_figures.py` transactionally publish owned outputs and the manifest last.
The orchestrator only performs output
...
write the numerics section. A manifest status/history update is accepted only
after the evidence above validates; it never substitutes for that validation.

Package-Scribe follows the same ownership boundary for calculation results:
`finalize_package_result.py` validates the complete owned attempt and
transactionally publishes the task directory plus manifest, with the manifest
last. The orchestrator only validates and reports the returned event.

Foundation skills follow the corresponding candidate boundary:
`init_foundation_attempt.py` allocates owner/mode-scoped private state and
`finalize_foundation_attempt.py` validates and transactionally publishes
`hep-idea` / `hep-paper-formalize` owner files plus the manifest last. An agent
must never treat candidate existence as completion or copy it into live paths.

---

## Manifest publication verification (orchestrator-owned coordination)

This orchestrator has no general-purpose direct manifest write procedure. For
every workflow state change:

1. Read the current manifest and capture the owner and intended scope
2. Dispatch the documented skill or contract-bound script writer
3. Re-read the published manifest and owner artifacts
4. Run `scripts/validate_workspace_projects.py`
5. Verify the expected scoped state/history event, exact dependencies, and
   preservation of unrelated owner state
6. If any check fails, report the publication as incomplete and retry or invoke
   the documented recovery path; do not patch fields, append history, or perform
   a second manifest write

Explicit migration remains owned by `scripts/migrate_manifest_v2.py`. If a
future workflow needs agent-owned manifest state, first define its narrow field
and event ownership across the schema, contracts, both agent mirrors, and tests,
then publish through a contract-bound transactional writer. Ad hoc agent
...
- Do not accept the publication or patch the manifest
- Ask user: "Retry the owning skill?" or "Stop this workflow step?"
